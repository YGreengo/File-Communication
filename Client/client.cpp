...
Client* my_client;
char version = '3';
vector<CryptoPP::byte> aes_key;
string username;
string address;
string port;
string transfer_file_path;
unsigned char request_1025_flag = 0, request_1026_flag = 0 ,request_1027_flag = 0, client_finish =0, resend = 0;
...
//writes priv.key file
void write_key_to_file(const string& content)
{
    const string& file_name = "priv.key";
    ...
    if (file.is_open())
    {
        ...
        file << content;
        file.close();
        cout << "key successfully written to " << file_name << endl;
    }
    else
    {
        cerr << "Error: Unable to open file " << file_name << " for writing." << endl;
    }
}

//reads priv.key file
string read_key_file(const string& file_name)
{
...
//turns hexadecimal to byte vector
vector<CryptoPP::byte> hex_to_bytes(const string& hex) {
    vector<unsigned char> bytes;
    //iterates to cast
    for (size_t i = 0; i < hex.length(); i += 2) {
        unsigned int byte;
        istringstream(hex.substr(i, 2)) >> std::hex >> byte;
        bytes.push_back(static_cast<unsigned char>(byte));
    }
    return bytes;
}

//loads me.info file details
void load_details() {
    string path = "transfer.info";
    if (file_exists(path)) {
        ifstream file(path);
        if (!file.is_open()) {
        ...
        //read port
        getline(file, port);
        //read the transfer file path
        getline(file, transfer_file_path);
        file.close();

        cout << "Address: " << address << ", Port: " << port << endl;
        cout << "Transfer File Path: " << transfer_file_path << endl;
    }
    else {
...
string AESEncrypt(const CryptoPP::byte* key, const char* plaintext, unsigned int length) {
    ...
    try {
        ...
        stfEncryptor.Put(reinterpret_cast<const CryptoPP::byte*>(plaintext), length);
...
string decryptRSA(const RSA::PrivateKey& privateKey, const vector<CryptoPP::byte>& ciphertext) {
    ...
    CryptoPP::StringSource(ciphertext.data(), ciphertext.size(), true,
        new CryptoPP::PK_DecryptorFilter(rng, decryptor, new CryptoPP::StringSink(plaintext)));

    ostringstream oss;
    oss << hex << setfill('0');
    for (size_t i = 0; i < plaintext.size(); ++i) {
        oss << setw(2) << static_cast<int>(static_cast<unsigned char>(plaintext[i]));
    }

    string hexPlaintext = oss.str();
    if (hexPlaintext.size() % 2 != 0) {
        hexPlaintext.insert(0, "0");
    }

    for (char& c : hexPlaintext) {
        c = tolower(c);
    }
...
bool check_username(const string& username) {
    // check if only alphanumeric chars are gives
    regex pattern("^[a-zA-Z0-9_]+$");

    // check if username length is within the allowed limit (<= 254 characters)
    if (username.size() > 254) {
        return false;
    }

    // if username matches the regular expression pattern
    return regex_match(username, pattern);
}

//converter to base 64
string base_64_convert(RSA::PrivateKey privateKey) {
    string encodedPrivateKey;
    ByteQueue privateKeyQueue;
    privateKey.Save(privateKeyQueue);
    Base64Encoder encoder;
    privateKeyQueue.CopyTo(encoder);
    encoder.MessageEnd();
    size_t size = encoder.MaxRetrievable();
    encodedPrivateKey.resize(size);
    encoder.Get((CryptoPP::byte*)&encodedPrivateKey[0], encodedPrivateKey.size());
    return encodedPrivateKey;
}

//writes me.info file
void write_my_file(const string& name, const string& id, const string& private_key) {
...
//makes private key object from string
RSA::PrivateKey remake_private_key(const string& key_string)
{
    // create a byte queue to hold the bytes of the key
    CryptoPP::ByteQueue queue;
    // put the bytes of the key string into the byte queue
    queue.Put(reinterpret_cast<const CryptoPP::byte*>(key_string.data()), key_string.size());
    queue.MessageEnd();
    // create an RSA private key object
    RSA::PrivateKey privateKey;
    //load private key from byte queue
...
string hex_to_string(const vector<unsigned char>& hexData) {
    ...
    stringstream ss;
    //iterate to cast and append
    for (size_t i = 0; i < hexData.size(); ++i) {
        ss << hex << setw(2) << setfill('0') << static_cast<int>(hexData[i]);
    }
    return ss.str();
}

//packs the request in the right format
vector<unsigned char> Request::pack() const {
    vector<unsigned char> request;
    request.insert(request.end(), client_id.begin(), client_id.end());
    request.push_back(version);
    unsigned short int code_little_endian = boost::endian::native_to_little(code);
    request.push_back(code_little_endian & 0xFF); // get the code LSB
    request.push_back(code_little_endian >> 8);   // get the code MSB
    unsigned int payload_size_little_endian = boost::endian::native_to_little(payload_size);
    request.push_back(payload_size_little_endian & 0xFF);       //get the payload size LSB
    request.push_back((payload_size_little_endian >> 8) & 0xFF);
    request.push_back((payload_size_little_endian >> 16) & 0xFF);
    request.push_back((payload_size_little_endian >> 24) & 0xFF); //get the payload size MSB
    request.insert(request.end(), payload.begin(), payload.end());
    return request;
}

//handles request 1025
void handle_1025_request(boost::asio::ip::tcp::socket& s) {
    if (request_1025_flag) {
    ...
    if (file_exists("me.info")) {
        cout << "me.info already exists. please log in with 1027 request" << endl;
        return;
    }
    username.resize(255);
    char reply_header[7];
    char payload[16];
    string user_id(16, '0');
    unsigned short int code = 1025;
    unsigned int payload_size = 255;

    Request request;
    request.set_client_id(user_id);
    request.set_version(3);
    request.set_code(code);
    request.set_payload_size(payload_size);
    request.set_payload(vector<char>(username.begin(), username.end()));

    // Pack the 1025 request in the right format
    vector<unsigned char> request_pack = request.pack();

    // Send the request to the server
    boost::asio::write(s, boost::asio::buffer(request_pack));

    // Get reply header
    boost::asio::read(s, boost::asio::buffer(reply_header, 7));
    // Extract reply code
    unsigned short int reply_code = (reply_header[1]) | (reply_header[2] << 8);
    // Error - prints a message
    if (reply_code == 1601) {
        cout << "the username " << username << " already exists. please try again with a different name" << endl;
        return;
    }
    ...
    my_client->set_name(username);
    my_client->set_id(user_id);
    my_client->set_private_key(privateKey);
    my_client->set_public_key(publicKey);
    // Encode key to base 64
    string encodedPrivateKey = base_64_convert(privateKey);
    // Read rest of message from the server - client id
    boost::asio::read(s, boost::asio::buffer(payload, sizeof(payload)));
    // Assign the client id to the client object
    my_client->set_id(string(payload, payload + sizeof(payload)));
    unsigned char reply_version = reply_header[0];
    // Turns the hex id to string to write in file
    string id = hex_to_string(vector<unsigned char>(payload, payload + sizeof(payload)));
    // Write me.info file
    write_my_file(username, id, encodedPrivateKey);
    // Write priv.key file
    write_key_to_file(encodedPrivateKey);
    // Wait a little
    this_thread::sleep_for(chrono::milliseconds(100));
}

//handles request 1026
void handle_1026_request(boost::asio::ip::tcp::socket& s, unsigned short int code) {
    if (!request_1025_flag) {
        cout << "you need to first register with a username before you exchange keys" << endl;
        return;
    }
    if (request_1026_flag) {
        cout << "you already exchanged keys." << endl;
        return;
    }
    Request request;

    // Set request client_id using setter method
    request.set_client_id(my_client->get_id());
    request.set_version(3);
    request.set_code(code);

    vector<Integer> payload;
    Integer modulus = my_client->get_public_key().GetModulus();
    Integer publicExponent = my_client->get_public_key().GetPublicExponent();
    RSA::PrivateKey privkey = my_client->get_private_key();
    payload.push_back(modulus);
    payload.push_back(publicExponent);
    char reply_header[7];
    char reply_payload[144];
    vector<char> serializedPayload;

    // encode each Integer(public keys - modulus and exponent) into a byte vector
    for (const auto& integer : payload) {
        // create a byte block to hold the encoded integer
        SecByteBlock encodedInteger;
        // determine the minimum size needed to encode the integer
        size_t encodedSize = integer.MinEncodedSize();
        // Resize the byte block to fit the encoded integer
        encodedInteger.resize(encodedSize);
        // encode the integer into the byte block
        integer.Encode(encodedInteger.data(), encodedInteger.size());
        // append the encoded Integer to the payload
        serializedPayload.insert(serializedPayload.end(), encodedInteger.begin(), encodedInteger.end());
    }

    // serialize client name into little-endian byte representation
    string paddedName = my_client->get_name();
    paddedName.resize(255, '\0');

    vector<char> serializedName(paddedName.begin(), paddedName.end());

    // concatenate serialized name and serialized payload
    vector<char> completePayload;
    completePayload.insert(completePayload.end(), serializedName.begin(), serializedName.end());
    completePayload.insert(completePayload.end(), serializedPayload.begin(), serializedPayload.end());

    // convert payload size to little-endian
    request.set_payload_size(boost::endian::native_to_little(static_cast<uint32_t>(completePayload.size())));

    // assign completePayload to request payload
    request.set_payload(completePayload);
    ...
    // read the reply
    boost::asio::read(s, boost::asio::buffer(reply_header, 7));
    // extract the reply code - little endian
    unsigned short int reply_code = (reply_header[1]) | (reply_header[2] << 8);
    // the server didn't find the client
    if (reply_code == 1606) {
        cout << "no client found in server for id: " << my_client->get_id() << " try again" << endl;
        return;
    }
    // server has found the client and got the keys
    cout << "keys exchanged successfully" << endl;
    request_1026_flag = 1;
    // read the rest of the message
    boost::asio::read(s, boost::asio::buffer(reply_payload, sizeof(reply_payload)));
    // read the client id
    string client_id_hex = hex_to_string(vector<unsigned char>(reply_payload, reply_payload + 16));
    // read the encrypted aes key
    string aes_encrypted_key_hex = hex_to_string(vector<unsigned char>(reply_payload + 16, reply_payload + 144));

    // convert AES encrypted key from hex string to bytes
    vector<CryptoPP::byte> aes_encrypted_key_bytes = hex_to_bytes(aes_encrypted_key_hex);

    // decrypt AES encrypted key using RSA private key
    string aes_key_hex = decryptRSA(privkey, aes_encrypted_key_bytes);
    // save the global aes key to client
    aes_key = hex_to_bytes(aes_key_hex);

    return;
}

//handles request 1027
void handle_1027_request(boost::asio::ip::tcp::socket& s) {
    // If already signed up
    if (request_1025_flag) {
        // If already exchanged keys
        if (request_1026_flag) {
            cout << "you are already logged in and have the aes key." << endl;
            return;
        }
        cout << "you are already signed up. now just exchange keys with server" << endl;
        return;
    }

    char reply_header[7];
    char reply_payload[144];
    string file_name = "me.info";
    ...
    if (file_exists(file_name)) {
        ...
        getline(file, client_id);

        file_name = "priv.key";
        // Read the key from the file in base 64
        string encoded_priv_key = read_key_file(file_name);
        // Decode the base64 
        string decoded_priv_key = Base64Wrapper::decode(encoded_priv_key);
        // Make private key object
        RSA::PrivateKey privateKey = remake_private_key(decoded_priv_key);
        // Create the public key to fit the private key
        RSA::PublicKey publicKey(privateKey);
        vector<CryptoPP::byte> client_id_bytes = hex_to_bytes(client_id);
        vector<char> client_id_char(client_id_bytes.begin(), client_id_bytes.end());
        string client_id_string(client_id_char.begin(), client_id_char.end());

        // Create the client object
        my_client = new Client(client_name, client_id_string, privateKey, publicKey);
        my_client->set_id(client_id);

        Request request;
        // Set request parameters
        request.set_version(3);
        request.set_code(1027);
        request.set_payload_size(255);
        string paddedName = client_name;
        paddedName.resize(255, '\0');
        vector<char> client_name_vec(paddedName.begin(), paddedName.end());
        // Load the client name to the payload
        request.set_payload(client_name_vec);
        // Load the client id to the payload
        request.set_client_id(client_id_string);
        // Packs the request in the right format 
        vector<unsigned char> request_pack = request.pack();
        ...
        if (reply_code == 1606) {
            cout << "no keys found for client id: " << my_client->get_id() << " try again" << endl;
            delete my_client;
            return;
        }
        request_1027_flag = 1;
        ...
        vector<CryptoPP::byte> aes_encrypted_key_bytes = hex_to_bytes(aes_encrypted_key_hex);
        // Decrypt the aes key
        string aes_key_hex = decryptRSA(privateKey, aes_encrypted_key_bytes);
        // Save the aes key
        aes_key = hex_to_bytes(aes_key_hex);
        return;
    }
...
void handle_1028_request(boost::asio::ip::tcp::socket& s) {
    ...
    // If did not exchange keys
    if (!request_1026_flag) {
        cout << "you need to exchange keys first" << endl;
        return;
    }
    ...
    request.set_client_id(my_client->get_id());
    request.set_version(3);
    request.set_code(1028);
    size_t size = filesystem::file_size(transfer_file_path);
    const char* file_buffer = read_file_into_buffer(transfer_file_path);
    // If the client id read, convert to bytes and assign to payload
    if (request_1027_flag) {
        // Get the client ID from the Client object
        string client_id = my_client->get_id();
        // Convert the client ID from hex to string
        vector<unsigned char> client_id_chars = hex_to_bytes(client_id);
        // Convert the character vector to string
        string client_id_string(client_id_chars.begin(), client_id_chars.end());
        // Set the client ID in the Request object
        request.set_client_id(client_id_string);
    }


    unsigned int orig_file_size = size;
    // Initialize empty iv
    CryptoPP::byte iv[AES::BLOCKSIZE];
    memset(iv, 0, AES::BLOCKSIZE);
    // Get a pointer to the aes key
    const CryptoPP::byte* aes_key_ptr = aes_key.data();
    // Encrypt the whole file
    string ciphertext = AESEncrypt(aes_key_ptr, file_buffer, orig_file_size);
    // Get the encrypted file size
    unsigned int content_size = ciphertext.size();
    // Get the number of total chunks
    unsigned int total_chunks = ceil(static_cast<double>(content_size) / 734);
    // Pad the file name to 255 bytes
    string padded_name = transfer_file_path;
    padded_name.resize(255, '\0');
    string chunk;
    unsigned short int chunk_num = 1;
    unsigned int chunk_offset = 0;
    // Iterates over the ciphertext until finished to send all the encrypted file data
    while (chunk_num <= total_chunks) {
        // Each chunk will be 734 because sending messages of 1024 
        unsigned int chunk_size = 734;
        // Calculate remaining data
        unsigned int remaining_size = content_size - chunk_offset;
        // Calculate if there is a remainder
        unsigned int actual_chunk_size = min(chunk_size, remaining_size);
        // Get the actual chunk size
        chunk = ciphertext.substr(chunk_offset, actual_chunk_size);

        // Pad the chunk with null bytes if it's the last chunk and its size is less than 734
        if (chunk_num == total_chunks && actual_chunk_size < 734) {
            chunk.resize(734, '\0');
        }

        vector<char> payload;
        // Insert the content size
        payload.insert(payload.end(), reinterpret_cast<const char*>(&content_size), reinterpret_cast<const char*>(&content_size) + sizeof(unsigned int));
        // Insert the original file size
        payload.insert(payload.end(), reinterpret_cast<const char*>(&orig_file_size), reinterpret_cast<const char*>(&orig_file_size) + sizeof(unsigned int));
        // Insert the chunk number
        payload.insert(payload.end(), reinterpret_cast<const char*>(&chunk_num), reinterpret_cast<const char*>(&chunk_num) + sizeof(unsigned short int));
        // Insert the total chunks number
        payload.insert(payload.end(), reinterpret_cast<const char*>(&total_chunks), reinterpret_cast<const char*>(&total_chunks) + sizeof(unsigned short int));
        // Insert the file name
        payload.insert(payload.end(), padded_name.begin(), padded_name.end());
        ...
        // Packs the request
        vector<unsigned char> request_pack = request.pack();
        // Send the pack to server
        boost::asio::write(s, boost::asio::buffer(request_pack));
        // Keep count of chunk number
        ++chunk_num;
        // Calculate the offset for the buffer
        chunk_offset += actual_chunk_size;
    }

    // After the loop
    char reply_header[7];
    char reply_payload[279];
    // Read the reply header
    boost::asio::read(s, boost::asio::buffer(reply_header, 7));
    // Convert the reply code from little endian
    unsigned short int reply_code = (reply_header[1]) | (reply_header[2] << 8);
    // Get the rest of the message
    boost::asio::read(s, boost::asio::buffer(reply_payload, sizeof(reply_payload)));

    unsigned long server_file_crc_le;
    memcpy(&server_file_crc_le, reply_payload + 275, sizeof(server_file_crc_le));
    // Convert the server crc from little endian
    unsigned long server_file_crc = boost::endian::little_to_native(server_file_crc_le);
    // Calculate the client's original file crc
    unsigned long client_file_crc = memcrc(const_cast<char*>(file_buffer), orig_file_size);
    // If crc's match
    if (to_string(server_file_crc) == to_string(client_file_crc)) {
        cout << "file sent successfully\nclient CRC: " << to_string(client_file_crc) << "\nserver CRC: " << to_string(server_file_crc) << "\nCRC verified\n" << "going for 1029 request" << endl;
        handle_1029_30_31_request(s, 1029);
    }
    // They don't match
    else {
        // Keep track of resends
        ++resend;
        // Limit of 3
        if (resend == 4) {
            handle_1029_30_31_request(s, 1031);
            return;
        }
        // Not yet got to the limit
        cout << "crc mismatch\nclient CRC is:" << client_file_crc << "\nand server CRC is: " << server_file_crc << "\ntrying for the " << to_string(resend) << " time" << endl;
        handle_1029_30_31_request(s, 1030);
    }
}

//handles requests 1026, 1030 and 1031
void handle_1029_30_31_request(boost::asio::ip::tcp::socket& s, unsigned short int request_num) {
    char reply_header[7];
    char received_payload[16];
    Request request;
    ...
    boost::asio::write(s, boost::asio::buffer(request_pack));
    // If request id 1030 - try sending it again
    if (request_num == 1030) {
        return handle_1028_request(s);
    }
    // Reads the reply code
    boost::asio::read(s, boost::asio::buffer(reply_header, 7));
    // Convert the reply code
    unsigned short int reply_code = (reply_header[1]) | (reply_header[2] << 8);
    // Reached the limit of resends
    if (request_num == 1031) {
        cout << "you have reached the limit of resends without crc confirmation for the file, there must be something wrong\nlogging out now\nBye bye " << username << endl;
        return;
    }
    // Got ack from server
    if (reply_code == 1604) {
        cout << "transfered file sent and saved\nnow logging out.\nBye Bye " << username << endl;
        boost::asio::read(s, boost::asio::buffer(received_payload, 16));
        client_finish = 1;
        return;
    }
}
int main() {
    try {
        ...
        for (;;) {
            if (client_finish == 1 || resend == 4)
                return 0;
            cout << "Enter request number: ";
            char request[512];
            //get input - request number
            cin.getline(request, sizeof(request)); 

            if (strcmp(request, "1025") == 0) {
                handle_1025_request(s);
            }
            else if (strcmp(request, "1026") == 0) {
                handle_1026_request(s, 1026);
            }
            else if (strcmp(request, "1027") == 0) {
                handle_1027_request(s);
            }
            else if (strcmp(request, "1028") == 0) {
                handle_1028_request(s);
            }
